...
public class RayClick : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Physics.Raycast(this.transform.position, forward, out hit) && !hit.transform.name.Equals("Wall") && !hit.transform.name.Equals("Plane") && !isMoving && hit.transform.name != name)
        {
    ...
    {
        if (other.transform.name.Equals(name))
        {
            isMoving = false;
        }
    }
